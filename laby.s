...
### commande pour éxecuter depuis une ligne de commande et écrire le labyrinthe dans un fichier : java -jar Mars4_5.jar laby.s pa <N> | sed '1,2d' | sed '$d' > laby.txt
...
# fonction non-utilisee, a la place on utilise la fonction neighbour_position qui suit.
lab_direction_ind: 
#prologue
addi $sp $sp -16
sw $ra 0($sp)
sw $a0 4($sp) 			# adresse du labyrinthe
...
# fonction qui retourne le bit du mur a casser
neighbour_position:
#prologue
addi $sp $sp -12
sw $ra 0($sp)
