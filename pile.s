...
li $t0, 0
move $t5, $a0 # pour faire le parcours du tableau $t5 contient l'adresse courante
for_st_sommet: beq $t0 $a1 st_sommet # si on atteint la fin du tableau alors on prend le sommet: $t5 -> $t5 - 4
	       lw $t1 0($t5) # la valeur du element courant du tableau -> $t1
	       beqz $t1 st_sommet # si $t1 == 0, on prend le sommet $t5 -> $t5 - 4
	       addi $t0, $t0, 1 # compteur d'elements -> $t0
	       addi $t5, $t5, 4 # adresse du element suivant -> $t5
	       b for_st_sommet
st_sommet: addi $t5 $t5 -4 # on revient sur la bonne adresse qui est $t5 -> $t5 - 4
